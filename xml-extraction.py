...
class file:
    ...
    def __init__(self, filePath=None, newspaper=None, date=None, pageNumber=None, pageConfidence=None, OCRSoftware=None, text=None):
...
def createDatabase(dbname):
    ...
    cursor.execute("""CREATE VIRTUAL TABLE IF NOT EXISTS pages_fts
                   USING fts5(text, content="pages", content_rowid="rowid", tokenize="trigram")""")
    ...
    cursor.executemany("INSERT INTO pages VALUES (?, ?, ?, ?, ?, ?, ?)", directoryParse(root_location, cursor)) # raw string
    cursor.execute("INSERT INTO pages_fts(rowid, text) SELECT rowid, text FROM pages") # pages_fts is basically an index on pages
    connection.commit()
    connection.close()

def directoryParse(dirRootPath, cursor):
    ...
    for dirRoot, dirName, files in os.walk(dirRootPath):
        for fileName in files:
            if (fileName.lower().endswith("xml")):
                ...
                cursor.execute("SELECT 1 FROM pages WHERE filepath = ?", filePath) # don't need to select all row data to check existence
                ...
                yield (cF.filePath, cF.newspaper, cF.date, cF.pageNumber, cF.pageConfidence, cF.OCRSoftware, cF.text) # yield generator to insert file by file
...
createDatabase("test_tnw.db") # As I run this, I only have the tnw_ShinSekai_The New World folder inside the relative directory \UCB